...
class Sober(nn.Module):
    ...
    def forward(self, x):
        self.sobel_kernel_x = torch.from_numpy(self.kernel_x).unsqueeze(0).expand(self.channels, 1, 3, 3, 3).float().cuda(device=x.device)
        self.sobel_kernel_y = torch.from_numpy(self.kernel_y).unsqueeze(0).expand(self.channels, 1, 3, 3, 3).float().cuda(device=x.device)
        self.sobel_kernel_z = torch.from_numpy(self.kernel_z).unsqueeze(0).expand(self.channels, 1, 3, 3, 3).float().cuda(device=x.device)
