...
from pangteen.network.medsegmamba.vss3d import VSSLayer3D
...
class MedSegMamba(nn.Module):
    def __init__(
            ...
            scan_type='scan',
            id=False,
            # whether to have conv1x1 in the residual path when input and output channel sizes of residual block is the same
            ...
            # found to do better when the last resblock is large on TABSurfer, but a lot more memory
            **kwargs
    ):
        super().__init__()
        ...
        self.preconv = nn.Conv3d(input_channels, channel_sizes[1], kernel_size=3, stride=1, padding=1) if preact else (
            lambda x: x)
        if preact:
        ...
        #     self.bottleneck = TransformerBottleneck((img_dim // patch_dim), channel_sizes[5], num_heads=16,
        #                                             num_layers=num_layers, dropout_rate=mlp_dropout_rate,
        ...
        if full_final_block:
            ...
            self.Conv_1x1 = nn.Conv3d(channel_sizes[2], num_classes, kernel_size=1, stride=1, padding=0)
        elif num_classes <= channel_sizes[1]:
            self.Up_conv2 = ResConvBlock3D(ch_in=channel_sizes[2], ch_out=channel_sizes[1], id=id, preact=preact)
            self.Conv_1x1 = nn.Conv3d(channel_sizes[1], num_classes, kernel_size=1, stride=1, padding=0)
        else:
            self.Up_conv2 = ResConvBlock3D(ch_in=channel_sizes[2], ch_out=(num_classes + 8 - num_classes % 8), id=id,
                                           preact=preact)
            self.Conv_1x1 = nn.Conv3d((num_classes + 8 - num_classes % 8), num_classes, kernel_size=1, stride=1, padding=0)

        self.act = nn.Softmax(dim=1)

        self.img_dim = img_dim
        self.patch_dim = patch_dim
        self.img_ch = input_channels
        self.output_ch = num_classes
...
class MedSegMamba_v2(nn.Module):  # add vss3d modules in skip connections
    def __init__(
        ...
        self.preconv = nn.Conv3d(img_ch, channel_sizes[1], kernel_size=3, stride=1, padding=1) if preact else (
            lambda x: x)
        if preact:
            self.Conv1 = ResConvBlock3D(ch_in=channel_sizes[1], ch_out=channel_sizes[1], id=id, preact=preact)  # 96
        else:
        ...
        if skip_layers[2] > 0:
            self.skip3 = VSSLayer3D(dim=channel_sizes[3],
                                    ...
                                    scan_type=scan_type,
                                    size=96 // 4,
                                    orientation_order=orientations[
                                                      skip_layers[0] + skip_layers[1]:skip_layers[0] + skip_layers[1] +
                                                                                      skip_layers[2]])
        ...
        if full_final_block:
            ...
            self.Up_conv2 = ResConvBlock3D(ch_in=channel_sizes[2], ch_out=(output_ch + 8 - output_ch % 8), id=id,
                                           preact=preact)
...
if __name__ == '__main__':
    model = MedSegMamba(**cfg.default_network_args).cuda()
    NetworkAnalyzer(model).analyze()