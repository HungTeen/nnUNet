...
from monai.networks.nets.swin_unetr import get_window_size, window_partition, window_reverse, rearrange, compute_mask
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask):
        b, n, c = x.shape
        qkv = self.qkv(x).reshape(b, n, 3, self.num_heads, c // self.num_heads).permute(2, 0, 3, 1, 4)  # B N C -> B N 3*C -> 3 B H N C/H
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward_part1(self, x, mask_matrix):
        ...
        if any(i > 0 for i in shift_size):
            ...
            shifted_x = x
            attn_mask = None
        x_windows = window_partition(shifted_x, window_size)  # B D H W C -> _ S^3 C
