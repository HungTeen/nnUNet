...
def conver_test_set(image_folder, image_id, test_str):
...
def conver_train_set(image_folder, label_folder, image_id, image_str, label_str):
    shutil.copy(join(label_folder, image_id), join(label_str, image_id))
...
def convert_kits2023(image_folder_name: str, label_folder_name: str, nnunet_dataset_id: int = 800):
    ...
    for i in test_set_ids:
        p.apply_async(conver_test_set, args=(image_folder, i, test_str))
    for i in train_set_ids:
        p.apply_async(conver_train_set, args=(image_folder, label_folder, i, image_str, label_str))
...
if __name__ == '__main__':
    '''
    python -u pangteen/dataset/Dataset800_KiTS23.py image label -d 888
    nohup python -u pangteen/dataset/Dataset800_KiTS23.py image label -d 888 > main3.out 2>&1 &
    '''
    import argparse
    ...
    convert_kits2023(args.input_folder, args.label_folder, args.d)
